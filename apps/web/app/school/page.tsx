...
function isProfileSubjectSelected(
  profile: CertificateProfileDto,
  subjectId: string,
): boolean {
...
function isCommentCategorySelected(
  cert: CertificatePrefs,
  categoryId: string,
): boolean {
  return commentPerGradeForCategory(cert, categoryId);
}

function countCommentCategories(
  cert: CertificatePrefs,
  total: number,
): number {
...
export default function SchoolPage() {
  ...
  const subjectsByParentCategory = useMemo((): ProfileSubjectParentGroup[] => {
    ...
    const groups: ProfileSubjectParentGroup[] = [...parentMap.entries()]
  ...
  const hasAttendance =
    cert.absences || cert.lateness || cert.hourAbsences || cert.hourLateness;

  const parentCategories = useMemo(
    () =>
      subjectsByParentCategory.map((g) => ({
        id: g.parentId,
        label: g.parentLabel,
      })),
                ...
                {subjectsByParentCategory.map((parent) => (
                  <div key={parent.parentId}>
                    ...
                    {parent.subGroups.map((sub) => (
                      <div
                        key={sub.subCategoryId ?? `${parent.parentId}-root`}
                        className="mb-2"
                      >
          ...
          <Card>
            ...
            <FieldRow
              ...
              onToggle={() =>
                syncCertToProfile({
                  ...cert,
                  showSubCategoriesOnCertificate:
                    cert.showSubCategoriesOnCertificate === false,
                })
              }
            />
            <FieldRow
